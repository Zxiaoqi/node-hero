...
app.get("/list", (req, res) => {
    fs.readFile(path.join(__dirname, "/json/hero-list.json"), (err, listData) => {
        if (err) return err;
...
app.get("/delete", (req, res, next) => {
    ...
    if (id === '') {
        ...
        //判断id项是否存在
        if (isId) {
            let imgfile = isId.icon.split("/")[2];
            //删除指定文件
            fs.unlinkSync(getPath("imgs/" + imgfile));
...
app.post("/edit", (req, res, next) => {
    ...
    upload(req, res, (err) => {
        if (!!err) {
            ...
            if (name !== "" && skill !== "" && id !== "") {
                const dt = getFileData("json/hero-list.json");
                let imgfile = dt.find(item => item.id == id).icon.split("/")[2];
                fs.unlinkSync(getPath("imgs/" + imgfile));
                //map修改了dt数组
                dt.map(item => {
                    if (item.id == id) {
